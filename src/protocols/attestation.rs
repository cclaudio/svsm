...
fn add_nonce_to_buffer(
    ...
    let nonce_start = PhysAddr::from(nonce_gpa);
    let nonce_size = usize::from(nonce_size);
    verify_fits_in_one_page(nonce_gpa, nonce_size)?;
    let nonce_end = (nonce_start + nonce_size).page_align_up();

    let guard = PerCPUPageMappingGuard::create(nonce_start, nonce_end, 0)?;

    let map: MapGuard = MapGuard::new_private(nonce_gpa, u64::from(nonce_size))
...
fn verify_fits_in_one_page(vaddr: VirtAddr, size: usize) -> Result<(), SvsmReqError> {
    if vaddr.page_align() == (vaddr + size).page_align() {
        Ok(())
    } else {
...
fn verify_aligned(vaddr: VirtAddr, align: u64) -> Result<(), AttestationResult> {
...
fn handle_attest_services_request(vmsa: *mut Vmsa) {
...
fn handle_attest_single_service_request(params: &mut RequestParams) {
    let srv_operation_paddr = PhysAddr::from(params.rcx);

    // The service operation structure is not required to be page aligned
    let start = srv_operation_paddr.page_align();
    let offset = srv_operation_paddr.page_offset();
    let end = (srv_operation_paddr + size_of::<AttestSingleServiceRequest>()).page_align_up();

    let guard = PerCPUPageMappingGuard::create(start, end, 0)?;
    let vaddr = guard.virt_addr() + offset;

    let guest_page = GuestPtr::<AttestSingleServiceRequest>::new(vaddr);
    let request = guest_page.read()?;

    let service_uuid = Uuid::from(request.service_guid);
...
pub fn attestation_handle_request(call_id: u32, params: &mut RequestParams) -> Result<(), SvsmReqError> {
    match call_id {
